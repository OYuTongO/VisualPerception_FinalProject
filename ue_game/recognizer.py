...
if __name__ == "__main__":
    ...
    print("按 Q 退出")
    print(f"OSC target: {osc_host}:{osc_port}", flush=True)
    print("OSC progress address: /asl/progress", flush=True)
    print("OSC trigger address: /asl/trigger", flush=True)
    print("=" * 50, flush=True)
    with Recognizer() as rec:
        ...
        while True:
            ...
            if letter:
                ...
                recognized_char = str(letter).upper()
                expected_index = len(gate_cache)
                expected_char = gate_target[expected_index]
                if recognized_char == expected_char:
                    gate_cache.append(recognized_char)
                    progress_index = len(gate_cache) - 1
                    client.send_message("/asl/progress", int(progress_index))
                    print(
                        f"完整触发: {gate_target}, 当前cache中内容: "
                        f"{''.join(gate_cache)}, sent /asl/progress {progress_index}",
                        flush=True,
                    )
                else:
                    print(
                        f"顺序不匹配: 识别到 {recognized_char}, 当前期待 {expected_char}, "
                        f"当前cache中内容: {''.join(gate_cache) or '-'}",
                        flush=True,
                    )

                if "".join(gate_cache) == gate_target:
                    client.send_message("/asl/trigger", 1)
                    print(
                        f"[GATE] completed. sent /asl/trigger 1 to "
                        f"{osc_host}:{osc_port}",
                        flush=True,
                    )
                    gate_cache = []
